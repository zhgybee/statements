	...
	if(sessionuser != null)
	{
		...
		else if(mode.equals("2"))
		{
			...
			try
			{
				...
				if(data.size() == 0)
				{
					datasource.execute("insert into "+tablename+"(ID, XM, XMBH, "+columnname+", MODE, STATEMENT_ID, SUBSTATEMENT_ID, CREATE_USER_ID, CREATE_DATE) values(?, ?, ?, ?, ?, ?, ?, ?, NOW())", 
							SystemUtils.uuid(), itemname, itemcode, value, statementmode, statementId, substatementId, sessionuser.getId());
				}
				else
				{
					datasource.execute("update "+tablename+" set "+columnname+" = ?, CREATE_DATE = NOW() where STATEMENT_ID = ? and SUBSTATEMENT_ID = ? and XMBH = ?", value, statementId, substatementId, itemcode);
				}
				connection.commit();
			}
			catch(Exception e)
