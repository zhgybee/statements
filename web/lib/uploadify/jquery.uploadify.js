...
(function($) {
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					width           : 60,                // The width of the browse button
					// Events
				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						$('#' + settings.queueID).find('.uploadify-queue-item').each(function() {
							...
							$(this).delay(100 + 100 * delay).fadeOut(50, function() {
								$(this).remove();
							});
						});
						...
						for (var n = 0; n < args.length; n++) {
							...
							$('#' + args[n]).find('.data').removeClass('data').html(' - 删除');
							$('#' + args[n]).find('.uploadify-progress-bar').remove();
							$('#' + args[n]).delay(100 + 100 * n).fadeOut(50, function() {
								$(this).remove();
							});
						}
					}
					...
					$item.find('.data').removeClass('data').html(' - 删除');
					$item.find('.uploadify-progress-bar').remove();
					$item.delay(100).fadeOut(50, function() {
						$(this).remove();
					});
				}
			});
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					...
					/*
					var replaceQueueItem = confirm('文件['+file.name+']已经存在。是否重新上传？');
					if (!replaceQueueItem) {
		...
		onUploadSuccess : function(file, data, response) {
			...
			var cancel = $(".uploadify-queue-item[id='"+file.id+"']").find(".cancel a");  
			cancel.click
			(
				function(event) 
				{  
					...
					*/
					...
					delFile(settings.del, file.name, null);
				}
			);  
